    ...
    public func selectImage() {
        ...
        for (name, source) in actionsData {
            let action = UIAlertAction(
                title: .tr("FyreplaceKit.ImageSelector.chooseSource.action.\(name)"),
    ...
    private func extractImageData(image: UIImage, isPng: Bool) {
        ...
        guard data.count < maxBytes else {
            let alert = UIAlertController(
                title: .tr("FyreplaceKit.ImageSelector.sizeError.title"),
                message: .tr("FyreplaceKit.ImageSelector.sizeError.message"),
                preferredStyle: .alert
            )
            let ok = UIAlertAction(
                title: .tr("FyreplaceKit.ImageSelector.sizeError.action.ok"),
                style: .default
            )
